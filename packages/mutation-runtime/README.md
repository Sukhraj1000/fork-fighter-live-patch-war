...
The first runtime slice supports the contract composition used by Debt
Collector:
...
2. Call `activateMutation(runtime, unknownConfig, boundary)`. Activation parses
   ...
   emits `patch_activated`.
3. After each game-core transition, pass its state and events to
   `processMutationGameBoundary`. Matching core events spawn collectors and
   emit `patch_effect_applied`.
...
Contract-valid trigger, effect, and cleanup variants outside this initial
slice are rejected before activation with a typed `MutationRuntimeError`.
